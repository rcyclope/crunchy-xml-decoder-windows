...
def video():
    ...
    from altfuncs import jaJP
    if re.search('mp4', jaJP):
        cmd = ['.\\video-engine\\rtmpdump',
               '-r', url1, '-a', url2,
               '-f', 'WIN 11,8,800,50',
               '-m', '15',
               '-W', 'http://static.ak.crunchyroll.com/versioned_assets/StandardVideoPlayer.f3770232.swf',
               '-p', page_url2,
               '-y', filen,
               '-o', '.\\export\\{}.flv'.format(title)]
        error = subprocess.call(cmd)
    else: 
        cmd = '".\\video-engine\\ffmpeg" -i "{0}" -y -c copy -c:a aac -strict -2 ".\\export\\{1}.flv"'.format(jaJP, title)
...
'''
    ...
    # ----------
    if re.search('mp4', xmlconfig.find('file').string):
        try:
